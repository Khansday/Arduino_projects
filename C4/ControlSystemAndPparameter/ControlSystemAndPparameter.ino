...
float err, Kp = 25.00; // error of the PID system and P constant
...
void loop() {
 ...
 /* Serial.print("aX = "); Serial.print((accelerometer_x ));  //16384.0
  ...
  move_robot();  //new function to control the movement of the robot
...
void move_robot(){
  err = roll - 0;   //calculate eror
  pwm = err * Kp;   //apply the P parameter
  pwm = abs(pwm);  //make it always positive
  pwm = constrain(pwm, 0, 255);  //limit the value to match analogWrite
