...
double errO = 0;  //store previous reading for next cycle
...
void move_robot(){
  errd = err - errO;                      //rate of change of error 
  errO = err;                             //preparing for next cycle
  integral = integral + (Ki * err) ;      //keep adding the errors
  integral = constrain(integral, -2, 2);  //avoid the value to diverge
  
  pwm = err * Kp + errd * Kd + integral;  //value with PID loop
  pwm = abs(pwm);
